...
class RegisterDetailPage extends StatefulWidget {
  ...
  const RegisterDetailPage({required this.register, Key? key}) : super(key: key);
        ...
        child: Column(
          ...
          children: [
            StreamBuilder<DocumentSnapshot>(
              stream: FirebaseFirestore.instance.collection('$email-registers').doc(widget.register.registerName).snapshots(),
                ...
                return Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 8),
                      _buildInfoCard(
                        'Last readed value date',
                        '${formatDateTime(register.registerValue.last.date)}',
                      ),
                      SizedBox(height: 8),
                      _buildInfoCard(
                        'Readed Register Amount',
                        '${register.registerValue.length}',
                      ),
                      SizedBox(height: 16),
                      MyChart(
                        registerName: widget.register.registerName,
                      ),
                      SizedBox(height: 16),
                      Card(
                        elevation: 2,
                      ...
                      ListView.builder(
                        ...
                        itemCount: register.registerValue.length > 25 ? 25 : register.registerValue.length,
                        itemBuilder: (context, index) {
                          var reversedIndex = register.registerValue.length - index - 1;
